...
class LeverFormAdapter(BaseFormAdapter):
    """
    Adapter for inspecting and interacting with Lever application forms.
    """
    ...
    def inspect_form(self, html_content: str, url: str) -> List[FieldInspectionItem]:
        ...
        form = soup.select_one(".application-form, form#application-form, form")
        if not form:
            return fields

        field_containers = form.select(".application-question, .form-group, li.application-question, div[class*='application']")
        ...
        for container in field_containers:
            label_tag = container.select_one(".application-label, .text, label")
            ...
            fields.append(
                FieldInspectionItem(
                    ...
                    label=label_text.replace("✱", "").replace("*", "").strip(),
                    html_name=html_name,
